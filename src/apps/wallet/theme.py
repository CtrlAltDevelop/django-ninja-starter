...
admin with `Unfold <https://unfoldadmin.com>`_, and the payment-method screens
look native there because they use Unfold's layout, filters and decorators. A
project that has never heard of Unfold should still be able to drop this app in
and get a working -- if plainer -- admin, rather than an ``ImportError`` at
startup.
