"""Getting a saved notification out to the sockets that are waiting for one.

A notification is created in a request -- ordinary synchronous Django code --
and has to reach WebSocket connections that are being held open somewhere else
entirely, possibly in another worker process. That is a fan-out problem, and it
is the only genuinely hard part of this app, so it lives behind one small
...
client connected to the first never hears about a notification created by the
second. That is fine for development and wrong for production, which is why the
app's settings contract warns about it rather than leaving it to be discovered.
...
the code that creates a notification is: a view, a signal handler, a management
...
point of the socket's authenticate command, which adds a private channel to a
connection that already has the public one. Both implementations therefore
accept :meth:`add` at any time, including from another task.
"""
...
# `add` safe; the cost is that joining a channel can lag by this much.
POLL_SECONDS = 0.25


def channel_prefix() -> str:
    return settings.NOTIFICATIONS_CHANNEL_PREFIX


def global_channel() -> str:
    """The channel every connection joins, authenticated or not."""
    return f"{channel_prefix()}:global"


def user_channel(user_id: Any) -> str:
    """The channel only one account's connections join, once they have proven who they are."""
...
class RedisBroker:
    ...
    def client(self) -> Any:
        if self._client is None:
            ...
            self._client = redis.Redis.from_url(settings.NOTIFICATIONS_REDIS_URL)
        return self._client
    ...
    def async_client(self) -> "redis.asyncio.Redis":
        if self._async_client is None:
            ...
            self._async_client = redis.asyncio.Redis.from_url(settings.NOTIFICATIONS_REDIS_URL)
...
def get_broker() -> Broker:
    ...
    if _broker is None:
        try:
            _broker = import_string(settings.NOTIFICATIONS_BROKER)()
        except ImportError as error:
            raise ImproperlyConfigured(
                f"DJANGO_NOTIFICATIONS_BROKER is not importable: {settings.NOTIFICATIONS_BROKER}"
            ) from error
    return _broker
...
def _reset_on_setting_change(setting: str, **kwargs: object) -> None:
    if setting in {"NOTIFICATIONS_BROKER", "NOTIFICATIONS_REDIS_URL"}:
        reset_broker()
