...
from infrastructure.oauth.apple.provider import provider as apple
from infrastructure.oauth.core.social import OAuthProviderError, ProviderTokens
from infrastructure.oauth.core.tokens import hash_token
from infrastructure.oauth.github.provider import provider as github
from infrastructure.oauth.google.provider import provider as google
from infrastructure.oauth.microsoft.provider import provider as microsoft
...
def test_id_token_nonce_is_checked(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("infrastructure.oauth.core.provider.jwt.PyJWKClient", JwkClient)
    ...
    monkeypatch.setattr("infrastructure.oauth.core.provider.jwt.decode", decode)
...
def test_token_exchange_and_profile_request_use_bounded_http_calls(
    ...
    monkeypatch.setattr("infrastructure.oauth.core.provider.httpx.post", post)
    monkeypatch.setattr("infrastructure.oauth.core.provider.httpx.get", get)
...
def test_token_exchange_rejects_invalid_lifetime(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "infrastructure.oauth.core.provider.httpx.post", lambda *args, **kwargs: Response()
    )
...
def test_github_selects_primary_verified_email(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "infrastructure.oauth.github.provider.httpx.get", lambda *args, **kwargs: Response()
    )
