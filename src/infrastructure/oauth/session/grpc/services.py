"""The token operations for the session mode, over gRPC.
...
def _pb2() -> Any:
    from infrastructure.oauth.session.grpc import oauth_session_pb2

    return oauth_session_pb2


class OAuthSessionServiceRpc(generics.GenericService):
    """Refresh, revoke, list and end -- for the session mode."""
...
GRPC_SERVICES = [OAuthSessionServiceRpc]
...
# `/auth/token`.
GRPC_SERVED = settings.AUTH_TOKEN_MODE == "session"
