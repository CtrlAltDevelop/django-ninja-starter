"""The token operations for the rotation mode, over gRPC.
...
def _pb2() -> Any:
    from infrastructure.oauth.rotation.grpc import oauth_rotation_pb2

    return oauth_rotation_pb2


class RotationTokenServiceRpc(generics.GenericService):
    """Refresh, revoke, list and end -- for the rotation mode."""
...
GRPC_SERVICES = [RotationTokenServiceRpc]
...
# `/auth/token`.
GRPC_SERVED = settings.AUTH_TOKEN_MODE == "rotation"
