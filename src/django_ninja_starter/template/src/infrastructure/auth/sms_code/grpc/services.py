"""SMS-code authentication, over gRPC.
...
from infrastructure.auth.sms_code.services import sms_code_service
from infrastructure.common.grpc.errors import action

START_REQUEST = [{"name": "phone", "type": "string"}]
VERIFY_REQUEST = [
...
def _pb2() -> Any:
    from infrastructure.auth.sms_code.grpc import auth_sms_code_pb2

    return auth_sms_code_pb2
...
class SmsCodeService(generics.GenericService):
    ...
    async def SignupStart(self, request: Any, context: Any) -> Any:
        challenge = await sync_to_async(sms_code_service.start_signup)(
            context.http_request, phone=request.phone
        )
    ...
    async def SignupVerify(self, request: Any, context: Any) -> Any:
        result = await sync_to_async(sms_code_service.verify_signup)(
    ...
    async def LoginStart(self, request: Any, context: Any) -> Any:
        challenge = await sync_to_async(sms_code_service.start_login)(
            context.http_request, phone=request.phone
        )
        return _challenge(challenge)
    ...
    async def LoginVerify(self, request: Any, context: Any) -> Any:
        result = await sync_to_async(sms_code_service.verify_login)(
            context.http_request, ticket=request.ticket, code=request.code
        )
        return login_message(result, _pb2())
    ...
    async def Logout(self, request: Any, context: Any) -> Any:
        detail = await sync_to_async(sms_code_service.logout)(
            context.http_request, token=request.token
        )
        return _pb2().Message(detail=detail)


GRPC_SERVICES = [SmsCodeService]
