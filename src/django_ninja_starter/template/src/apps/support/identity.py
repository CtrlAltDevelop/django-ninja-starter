...
def user_summary(user: Any) -> dict[str, Any]:
    ...
    ``staff`` is here and not in the notification app's version of this
    function, because it is the one bit that changes what the socket will do:
    a client renders a support widget, an agent renders a queue, and both learn
    which they are from the frame they get on connect rather than from a second
    call.
    """
    return {
        "id": str(user.pk),
