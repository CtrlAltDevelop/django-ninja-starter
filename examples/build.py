...
1.  ``examples/env.example`` is copied in as the project's ``.env``, which is
    ...
    mode on, along with all five feature apps the template ships -- the CMS,
    notifications, the shop, the support desk and the wallet, whose tables,
    routes and WebSockets exist only once they are named.
