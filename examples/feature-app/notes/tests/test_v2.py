...
def test_the_list_carries_excerpts_instead_of_bodies(db: None) -> None:
    ...
    listed = client.get(V2, **headers).json()
...
def test_search_matches_the_title_or_the_body(db: None) -> None:
    ...
    found = client.get(f"{V2}?q=oat", **headers).json()
...
def test_the_window_pages_through_the_list(db: None) -> None:
    ...
    page = client.get(f"{V2}?limit=2&offset=2", **headers).json()
...
def test_both_versions_serve_the_same_rows(db: None) -> None:
    ...
    assert client.get(f"{V1}{created['id']}", **headers).json()["title"] == "Shared"
    assert client.get(f"{V2}{created['id']}", **headers).json()["title"] == "Shared"
