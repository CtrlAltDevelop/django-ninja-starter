...
def _account(client: Client, identifier: str = "ada") -> dict[str, str]:
    ...
    return {
        "HTTP_AUTHORIZATION": f"Bearer {response.json()['data']['credentials']['access_token']}"
    }
...
def test_a_note_round_trips(db: None) -> None:
    ...
    assert read.json()["data"]["title"] == "Groceries"
    assert read.json()["data"]["body"] == "Oat milk"
...
def test_patch_changes_only_what_was_sent(db: None) -> None:
    ...
    assert response.json()["data"]["title"] == "Final"
    assert response.json()["data"]["body"] == "Keep me"
...
def test_pinned_notes_sort_ahead_of_newer_ones(db: None) -> None:
    ...
    titles = [note["title"] for note in client.get(NOTES, **headers).json()["data"]]
