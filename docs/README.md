...
| Feature apps | [`cms`](cms.md), [`notifications`](notifications.md), [`shop`](shop.md), [`support`](support.md), [`wallet`](wallet.md), [`notes`](notes.md) |
...
All five ship **off** in a generated project, which is why a fresh `/api/docs`
lists neither the shop nor the wallet: an app nobody named has no tables, no
routes and no admin. Turn one on in `.env`, run `manage.py migrate`, restart, and
its group appears. This repository's own `.env.example` has all five on, so
